...
function Movie({route}) {
    ...
    return (
        <ScrollView style={styles.container}>
            ...
            {movie.trailers && movie.trailers[0] && movie.trailers[0].results && movie.trailers[0].results[0] && movie.trailers[0].results[0].key && (
                <Pressable
                    style={({pressed}) => [
                        {opacity: pressed ? 0.5 : 1},
                        styles.trailerButton,
                    ]}
                    onPress={() => navigation.navigate("Trailer", {trailerID: movie.trailers[0].results[0].key})}
                >
